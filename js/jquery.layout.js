 ...
 *
 * $Date: 2008-12-01 00:23:11 +0100 (lun, 01 dic 2008) $
 * $Rev: 187 $
 * 
 ...
 */
	...
	var initOptions = function () {
		...
		options.name = opts.name || opts.defaults.name || "";

		// remove default options that should be set 'per-pane' - and remove 'name' if exists
		$.each("name,paneSelector,resizerCursor,customHotkey,onopen,onclose,onresize".split(","),
	...
	var initPanes = function () {
		...
		$.each(c.allPanes.split(","), function() {
			...
			$P = $Ps[pane] = $Container.children(o.paneSelector);
			if (!$P.length) {
				$Ps[pane] = false; // logic
 ...
 */
	...
	function keyDown (evt) {
		...
		else if (isCursorKey && o[PANE[code]].enableCursorHotkey) // valid cursor-hotkey
			pane = PANE[code];
